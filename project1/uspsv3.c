...
File *get_proc_file_pid ( char *process, char *extension ) { // extension is folder name such as "fdinfo", "sched"
    FILE *procFile = NULL;

    char filename[ DEFAULT_BUFF ];
...
void get_proc_status_data ( FILE * procFile ) {
    ...
    while ( processName == NULL && Stat == NULL && VMPeak == NULL && VMSize == NULL ) {
        if ( getline( &line, &len, procFile ) == -1 ) {
            if ( processName == NULL ) 
                processName = p1strdup ("NOT FOUND" );

            if ( state == NULL ) 
                state = p1strdup ("NOT FOUND" );
            
            if ( vmPeak == NULL ) 
                vmPeak = p1strdup ("NOT FOUND" );

            if ( vmSize == NULL ) 
                vmSize = p1strdup ("NOT FOUND" );
        }
    }
